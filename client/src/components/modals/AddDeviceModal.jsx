...
const AddDeviceModal = ({ isOpen, onClose, onSuccess }) => {
  const { createDevice, loading, error } = useDevice();
  ...
  if (!canOnboardDevice) {
    return (
      <AccessDeniedModal
        isOpen={isOpen}
        onClose={onClose}
        message="You don't have permission to onboard devices."
      />
    );
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitError(error.message || 'Failed to create device');
    } finally {
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end space-x-3 pt-6 border-t">
          ...
          >
            {isSubmitting || loading ? 'Creating Device...' : 'Create Device'}
          </button>
        </div>
      </form>
    </Modal>
