...
export const getP3DeviceDetails = asyncHandler(async (req, res) => {
  try {
    ...
    // Get the latest device status (for Current Device Status card)
    const latestDeviceStatusQuery = `
      SELECT TOP 1
        p3.CreatedAt,
        p3.Signal_Strength,
        CASE
    ...
    const latestDeviceStatusResult = await pool.request()
      .input('deviceId', sql.NVarChar, deviceData.Device_ID)
      .query(latestDeviceStatusQuery);

    const latestDeviceStatus = latestDeviceStatusResult.recordset[0] || null;
    ...
    const responseData = {
      ...
      latest_device_status: latestDeviceStatus ? {
        device_status: latestDeviceStatus.Device_Status,
        gsm_signal: latestDeviceStatus.Signal_Strength || 0,
        last_record_time: latestDeviceStatus.CreatedAt
      } : null
    };
