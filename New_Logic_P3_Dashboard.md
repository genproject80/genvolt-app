...
- [ ] Device ID display
- [ ] Entry ID display
- [ ] Record Time (formatted as locale date/time string)
- [ ] Last Event Date (always shows latest `MAX(CreatedAt)` for device, not current entry's date)
...
#### Device Health Card
- [ ] Current Status - Device Active (green dot) / Device Inactive (red dot)
- [ ] Communication - Connected (green badge) / Disconnected (yellow badge)
- [ ] Battery Status - Color-coded text (Full/Good=green, Low=yellow, Critical=red)
- [ ] Motor Status - Badge: Running (green), Stopped (gray), Fault (red)
...
|-----------|-------------|-------------|
| `P3DeviceInformationCard` | Device ID, Entry ID, Record Time, Last Event Date | `device_information` |
...
| `P3FaultDiagnosticsCard` | Current Status, Communication, Battery Status, Motor Status | `fault_diagnostics` |
...
**Note on Last Event Date:** The "Last Event Date" field in the Device Information card always shows the latest record date for the device (using `MAX(CreatedAt)` from `IoT_Data_Sick_P3`), not the current entry's date. This ensures users always see when the device last reported data, even when viewing historical entries.
...
| `P3DeviceInformationCard.jsx` | `client/src/components/p3DeviceDetail/` | Device info card |
...
| `P3FaultDiagnosticsCard.jsx` | `client/src/components/p3DeviceDetail/` | Fault diagnostics card |
...
*Last updated: 2026-01-05 (Added Quick Reference Feature Checklist for developer replication)*
